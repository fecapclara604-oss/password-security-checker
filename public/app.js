document.addEventListener('DOMContentLoaded', () => {
  ...
  async function submitCheck() {
    ...
    btnSubmit.innerHTML = '<span>⏳ Interceptando e Processando...</span>';
    ...
    try {
      ...
      // Atualiza visualizador do banco de dados
      ...
      btnSubmit.innerHTML = '<span class="btn-icon">⚡</span><span>Analisar e Salvar no Sistema</span>';
    }
  }
  ...
  async function clearDbRecords() {
    ...
    try {
      const res = await fetch('/api/passwords', { method: 'DELETE' });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
